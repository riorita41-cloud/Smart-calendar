<img width="1881" height="973" alt="1" src="https://github.com/user-attachments/assets/5a7bb9a3-9b39-48f4-9a1b-8039c17a36b0" />
Умный Календарь — приложение для эффективной подготовки к экзаменам с Pomodoro-таймером, системой опыта (XP) и автоматическим расписанием. Идеально подходит для студентов и тех, кто готовится к сертификациям.

## 🌟 Основные функции
Pomodoro-таймер с отслеживанием сессий и статистикой

Система опыта (XP) за выполненные задачи, расписание и ежедневные бонусы

Автоматическое расписание подготовки к экзаменам

Управление задачами с возможностью привязки к экзаменам

Кастомизация аватара с уникальными параметрами

История активности — отслеживайте свой прогресс

## ⚙️ Требования
PHP 8.1+

Composer

MySQL или PostgreSQL

Web-сервер (Nginx/Apache) или Symfony Web Server

## 🚀 Установка
### 1. Клонирование репозитория

```powershell
git clone https://github.com/riorita41-cloud/Smart-calendar.git
cd Smart-calendar
```

### 2. Установка зависимостей

```powershell
composer install
```

### 3. Настройка окружения

```powershell
# Для Windows
copy .env.example .env

# Для Linux/macOS
cp .env.example .env
```
Отредактируйте файл .env, указав параметры вашей базы данных:
```powershell
DATABASE_URL="mysql://db_user:db_password@127.0.0.1:3306/db_name?serverVersion=8.0"
```

### 4. Выполнение миграций

```powershell
php bin/console doctrine:migrations:migrate
```

### 5. Запуск приложения

```powershell
php bin/console server:run
```

## 📁 Структура проекта

```text
smart-calendar/
├── src/                  # Основной код приложения
│   ├── Controller/       # Контроллеры (логика маршрутов)
│   ├── Entity/           # Сущности базы данных (ORM)
│   ├── Repository/       # Репозитории (запросы к БД)
│   └── Service/          # Сервисы (бизнес-логика)
├── templates/            # Twig-шаблоны (HTML-страницы)
├── public/               # Публичные файлы (CSS, JS, изображения)
├── migrations/           # Миграции базы данных
├── config/               # Конфигурационные файлы
└── .env.example          # Пример переменных окружения
```

## 💡 Особенности системы XP

| Действие | Начисление XP |
| :--- | :---: |
| 🧠 Изучение вопроса | **+5** |
| ✅ Выполнение задачи | **+10** |
| 📅 Завершение дня по расписанию | **+30** |
| 🍅 Завершение Pomodoro-сессии | **+25** |
|  Ежедневный бонус | **+30** |

> ⚠️ **Важно:** Система гарантирует, что XP начисляется **только один раз** за каждое действие, даже если пользователь отменяет и повторяет действие.
